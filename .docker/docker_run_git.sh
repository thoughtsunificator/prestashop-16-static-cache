...
if [ $PS_DEV_MODE -eq 1 ]; then
	echo "\n* Disabling DEV mode ...";
	sed -ie "s/define('_PS_MODE_DEV_', fakse);/define('_PS_MODE_DEV_',\ true);/g" /var/www/html/config/defines.inc.php
fi
...
if [ $PS_DEBUG_PROFILING -ne 0 ]; then
		echo "\n* Enabling DEBUG PROFILING mode ...";
		sed -ie "s/define('_PS_DEBUG_PROFILING_', false);/define('_PS_DEBUG_PROFILING_',\ true);/g" /var/www/html/config/defines.inc.php
fi
